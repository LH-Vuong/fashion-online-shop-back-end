...
func (controller *OrderController) List(ctx gin.Context) {
	...
	infos, totalPages, err := controller.Service.ListByCustomerID(customerId, limit, offset)
...
func (controller *OrderController) Callback(ctx gin.Context) {
	...
	err = controller.Service.UpdatePaymentStatus(ctx.Param("payment_id"), dataJSON, payment.HandleCallback)
	if err != nil {
		return
	}
